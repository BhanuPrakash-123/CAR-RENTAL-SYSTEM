...
void write(vector<vector<string>> csv_data, string file_name) {
...
void printdata(const vector<vector<string>> &par) {
  // Find the maximum width of any word in the csv_data
  size_t max_width = 0;
...
/*------------------------ login, registration and Display menus for different
 * types of users--------------------*/
...
void User ::display_menu() {
  ...
  if (c == '1') { // logs in a user
    ...
    cout << "Enter password : ";
    cin >> _password;
    cout << "Enter type of the user : 1 if Customer, 2 if Employee, 3 if "
            "Manager : \n";
    ...
    if (_type == "1") {
      fstream fout("Database/Customers.csv", ios::app);
      fout << name << "," << _id << "," << _password << "," << user_record
           << '\n';
      cout << "New user has been added.\n";
    }
    if (_type == "2") {
      fstream fout("Database/Employee.csv", ios::app);
      fout << name << "," << _id << "," << _password << "," << user_record
           << '\n';
      cout << "New user has been added.\n";
    }
    if (_type == "3") {
      fstream fout("Database/Manager.csv", ios::app);
      fout << name << "," << _id << "," << _password << "," << user_record
           << '\n';
      cout << "New user has been added.\n";
    }
    if (_type == "1" || "2" || "3") {
      fstream fout("Database/Users_info.csv", ios::app);
...
void User ::login() {
  ...
  fstream file("Database/Users_info.csv", ios::in);
  int count = 0;
  if (file.is_open()) {
      ...
      if (words_in_a_row[1] == id) {
        ...
        if (words_in_a_row[2] == password) {
          ...
          while (password != words_in_a_row[2]) {
            cout << "You entered the wrong password. Enter 1 to reenter the "
                    "password and 2 to exit\n";
            char c;
            cin >> c;
            if (c == '1') {
              cout << "Enter the password : ";
...
void Car ::Show_duedate(string reg_no) {
  ...
  readfile("Database/all_cars_data.csv");
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][3] == reg_no) {
      flag = 1;
  ...
  string uid;
  time_t stamp;
  readfile("Database/rented_cars_data.csv");
  int f = 0;
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][2] == reg_no) {
      f = 1;
      uid = csv_data[i][0];
      stamp = stoi(csv_data[i][3]);
      break;
    }
  }
  if (f == 0) {
  ...
  csv_data.clear();
  int days = 30;
  readfile("Database/all_users_info.csv");
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][1] == uid) {
      if (csv_data[i][3] == "1")
  ...
  if (flag == 0)
    ...
    cout << due_time->tm_mday << "/" << 1 + due_time->tm_mon << "/"
         << 1900 + due_time->tm_year << "\n";
  }
}
...
void User ::see_all_cars(string id) {
  ...
  csv_data.clear();
  cout << "In the csv_data below, 0 signifies that the car is not rented and 1 "
          "means that the car is rented.\n";
  readfile("Database/all_cars_data.csv");
  printdata(csv_data);
  csv_data.clear();
}
...
void User ::rent_car(string id, string car_name, string type_user,
  ...
  if (stoi(get_userRecord(id)) < 35) {
    ...
    // threshold score represents the minimum acceptable record for renting more
    // than x number of cars
    if (count > 4 &&
        stoi(get_userRecord(id)) <
            75) // threshold score = 75 for renting more than 5 cars
    {
      cout << "You cant rent more than 5 cars as your record score is less "
              "than 75 !\n\n";
    }
      ...
      readfile("Database/all_cars_data.csv");
      for (int i = 0; i < csv_data.size(); i++) {
        if (csv_data[i][0] == car_name && csv_data[i][4] == "0") {
          flag = 1;
          ...
          upd.push_back(csv_data[i][3]);
          upd.push_back(to_string(time(0)));
          append(upd, "Database/rented_cars_data.csv");
      ...
      write(csv_data, "Database/all_cars_data.csv");
      csv_data.clear();
    }
  } else if (type_user == "2") {
    int flag = 0;
    ...
    if (count > 7 &&
        stoi(get_userRecord(id)) <
            80) // threshold score = 80 for renting more than 5 cars
    {
      cout << "You cant rent more than 7 cars as your record score is less "
              "than 80 !\n\n";
    }
...
void User ::return_car(string id, string reg_no) {
  ...
  readfile("Database/rented_cars_data.csv");
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][2] == reg_no && csv_data[i][0] == id) {
      f = 1;
      csv_data.erase(csv_data.begin() + i, csv_data.begin() + i + 1);
      cout << "car was rented by you and is now being returned\n";
      write(csv_data, "Database/rented_cars_data.csv");
      csv_data.clear();
      readfile("Database/all_cars_data.csv");
      for (int i = 0; i < csv_data.size(); i++) {
        if (csv_data[i][3] == reg_no)
...
string User ::get_userRecord(string id) {
  ...
  csv_data.clear();
  readfile("Database/Users_info.csv");
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][1] == id) {
      userRecord = csv_data[i][4];
      break;
    }
  }
...
void User ::update_userRecord(
    string id, string car_condition,
    string user_type) { // user_record varies from 0 and 100
                        // larger the user_record, the better the user is
                        // eligible to rent a car initially all users are given
                        // 100 as base record
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][0] == id) {
      int cur_time = time(0);
      int rent_time = stoi(csv_data[i][3]);
  ...
  // initially when giving for rent , the car is either good or excellent  in
  // condition
  int damage_score = 0;
  ...
  int new_userRecord =
      curr_userRecord - (days_exceeded * w1 + damage_score * w2);
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][1] == id) {
      csv_data[i][4] = to_string(new_userRecord);
      break;
    }
  }
...
void User ::see_rented_cars(string id) {
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][0] == id) {
      cout << count << ". ";
      count++;
      for (int k = 0; k < csv_data[i].size() - 1; k++)
        cout << csv_data[i][k] << " | ";
      time_t stamp = stoi(csv_data[i][3]);
      tm *dateformat = localtime(&stamp);
      cout << dateformat->tm_mday << "/" << 1 + dateformat->tm_mon << "/"
           << 1900 + dateformat->tm_year << "\n";
    }
  }
  if (count == 0)
...
void User ::check_available(string id, string carname) {
  ...
  int count = 1;
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][0] == carname && csv_data[i][4] == "0") {
      flag = 1;
      cout << count << ". ";
      count++;
      cout << "car is available!\n";
      for (auto j : csv_data[i])
        cout << j << " | ";
      cout << "\n";
      break;
    }
...
int User ::calc_fine(string id, string type_user) {
  ...
  if (type_user == "1") {
    ...
    for (int i = 0; i < csv_data.size(); i++) {
      if (csv_data[i][0] == id) {
        reg_nos.push_back(csv_data[i][2]);
        int cur_time = time(0);
        int rent_time = stoi(csv_data[i][3]);
        if ((cur_time - rent_time) / 86400 >
            10) // customer can rent a car for a max of 10 days
          fine += 100 * ((cur_time - rent_time) / 86400 -
                         10); // fine for customer is 100$ per day
      }
    }
    ...
    for (int i = 0; i < csv_data.size(); i++) {
      for (int j = 0; j < reg_nos.size(); j++) {
        if (csv_data[i][3] == reg_nos[j]) {
          rent_price += stoi(csv_data[i][1]);
        }
      }
    }

    cout << "Your Total fine(i.e., fine + rent price) is : "
         << fine + rent_price << "\n";
  ...
  if (type_user == "2") {
    ...
    for (int i = 0; i < csv_data.size(); i++) {
      if (csv_data[i][0] == id) {
        ...
        int rent_time = stoi(csv_data[i][3]);
        if ((cur_time - rent_time) / 86400 >
            20) // Employee can rent a car for a max of 20 days
          fine += 50 * ((cur_time - rent_time) / 86400 -
                        20); // fine for employee is 50$ per day
      }
    }
    int rent_price = 0;
...
void Manager ::see_all_users(string id) {
  ...
  csv_data.clear();
  readfile("Database/Users_info.csv");
  cout << "Below are all the users. 1 at the end signifies a Customer, 2 means "
          "a Employee and 3 means a Manager \n\n";
  cout << "The csv_data is given in the form of Customer, username, type of "
          "user.\n\n";
  print_user_info(csv_data);
  csv_data.clear();
}
void Manager ::see_all_cars(string id) {
  ...
  csv_data.clear();
  readfile("Database/all_cars_data.csv");
  cout << "Below are all the cars. 1 at the end signifies a Customer, 2 means "
          "a Employee and 3 means a Manager \n\n";
  cout << "The csv_data is given in the form of car name, car rent price, car "
          "condition, registration number, and is_rented(1 if car is "
          "rented).\n\n";
  printdata(csv_data);
  csv_data.clear();
}
void Manager ::add_user(string id) {
  ...
  cout << "Enter type of the user : 1 if Customer, 2 if Employee, 3 if Manager "
          ": \n";
  cin >> _type;
  fstream fout("Database/Users_info.csv", ios::out | ios::app);
  fout << name << "," << _id << "," << _password << "," << _type << '\n';
  cout << "New user has been added.\n";
}
void Manager ::add_car(string id) {
  ...
  cout << "Enter reg_no : ";
  cin >> reg_no;
  fstream fout("Database/all_cars_data.csv", ios::out | ios::app);
  fout << car_name << "," << rent_price << "," << car_condition << "," << reg_no
       << ",0" << '\n';
...
void Manager ::delete_user(string id) {
  ...
  write(csv_data, "Database/Users_info.csv");
  csv_data.clear();
  vector<string> reg_nos;
  readfile("Database/rented_cars_data.csv");
  for (int i = 0; i < csv_data.size(); i++) {
...
void Manager ::Search_user(string id) {
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][1] == uid) {
      ...
      cout << "User Name :" << csv_data[i][0] << endl;
      cout << "User Record :" << csv_data[i][4] << endl;
...
void Manager ::update_user(string id) {
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][1] == new_id) {
      ...
      if (c == '1') {
        csv_data[i][2] = new_field;
      } else if (c == '2') {
        csv_data[i][0] = new_field;
      } else {
        cout << "Invalid input!\n\n";
      }
      break;
...
void Manager ::Search_car(string id) {
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][3] == reg_no) {
      flag = 1;
      cout << "car name :  " << csv_data[i][0] << endl;
      cout << "car rent :  " << csv_data[i][1] << endl;
      cout << "car Condition :  " << csv_data[i][2] << endl;
      if (csv_data[i][3] == "0")
...
void Manager ::update_car(string id) {
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][3] == reg_no) {
      ...
      if (c == '1') {
        csv_data[i][0] = new_value;
      } else if (c == '2') {
        csv_data[i][1] = new_value;
      } else if (c == '3') {
        csv_data[i][2] = new_value;
      } else {
        cout << "Invalid input!\n\n";
      }
      break;
...
void Manager ::see_rented_to_user(string id, string uid) {
  ...
  if (reg_nos.size() == 0)
    ...
    for (int d = 0; d < reg_nos.size(); d++) {
      for (int i = 0; i < csv_data.size(); i++) {
        if (csv_data[i][3] == reg_nos[d]) {
          cout << count << ". ";
          count++;
          for (auto y : csv_data[i]) {
            if (y != csv_data[i].back())
              cout << y << " | ";
            else
              cout << y << "\n";
          }
...
void Manager ::see_rented_car(string id, string reg_no) {
  ...
  for (int i = 0; i < csv_data.size(); i++) {
    if (csv_data[i][2] == reg_no) {
      cout << "car with id " << reg_no << " and name " << csv_data[i][1]
           << " is rented to user with id : " << csv_data[i][0] << "\n";
      count = 1;
      return;
    }
  }
